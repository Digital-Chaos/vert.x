...
You can also send request with a body.
...
The `send` method is a facility for send requests.
...
handler on the future can be racy when this is done outside of the event-loop.
...
will ensure that events are processed sequentially.
